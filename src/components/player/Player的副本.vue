        ...
        <div class="bottom">
          ...
          <div class="operators">
            ...
            <div class="icon i-center">
              <i :class="playIcon" @click='togglePlaying' @touchstart.prevent='togglePlaying'></i>
            </div>
    ...
    <div class="mini-player" v-show='!fullScreen' @click='open'>
      ...
      <div class="control">
        <progress-circle :radius='circleRadius' :percent='percent'>
          <!-- 要阻止冒泡 -->
          <i :class="miniPlayIcon" class="icon-mini" @touchstart.prevent='togglePlaying' @click.stop="togglePlaying"></i>
        </progress-circle>
      </div>
      <!-- 歌曲列表 -->
      <div class="control">
  ...
  watch: {
    ...
    playing(newPlaying) {
      ...
      this.$nextTick(() => {
        ...
        // newPlaying ? audio.play() : audio.pause()
        this.audioTogglePlay(newPlaying)
      })
    }
  },
  methods: {
    audioTogglePlay(status) {
      const audio = this.$refs.audio
      if (status) {
